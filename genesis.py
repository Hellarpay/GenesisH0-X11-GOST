...
def get_args():
  ...
  parser.add_option("-a", "--algorithm", dest="algorithm", default="SHA256",
                    help="the PoW algorithm: [SHA256|scrypt|X11|X11-GOST|X13|X15]")
  ...
  if not options.bits:
    if options.algorithm == "scrypt" or options.algorithm == "X11" or options.algorithm == "X11-GOST" or options.algorithm == "X13" or options.algorithm == "X15":
...
def get_algorithm(options):
  supported_algorithms = ["SHA256", "scrypt", "X11", "X11-GOST", "X13", "X15"]
...
def generate_hash(data_block, algorithm, start_nonce, bits):
  ...
  while True:
    ...
    if is_genesis_hash(header_hash, target):
      if algorithm == "X11" or algorithm == "X11-GOST" or algorithm == "X13" or algorithm == "X15":
...
def generate_hashes_from_block(data_block, algorithm):
  ...
  elif algorithm == 'X11-GOST':
    try:
      exec('import %s' % "x11_gost_hash")
    except ImportError:
      sys.exit("Cannot run X11-GOST algorithm: module x11_gost_hash not found")
    header_hash = x11_gost_hash.getPoWHash(data_block)[::-1]
